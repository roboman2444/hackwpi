...
void sand_render(camera_t *c){
	...
	states_bindActiveTexture(0, GL_TEXTURE_2D, sandbuff.texture_id[2]);
	GLfloat mvp[16];
...
void sand_init(int numsands){
	...
	states_bindTexture(GL_TEXTURE1, 0);
	states_bindTexture(GL_TEXTURE0, sandbuff.texture_id[1]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, numsands, numsands, 0, GL_RGB, GL_FLOAT, spos);
	states_bindTexture(GL_TEXTURE0, sandbuff.texture_id[2]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, numsands, numsands, 0, GL_RGB, GL_FLOAT, spos);
	states_bindTexture(GL_TEXTURE0, sandbuff.texture_id[0]);
	int x, y;
	for(y = 0; y < numsands; y++){
	for(x = 0; x < numsands; x++){
		spos[(y * numsands + x) * 3+0] = (float)x / (float)numsands * 5.0;
		spos[(y * numsands + x) * 3+1] = (float)y / (float)numsands * 5.0;
